...
- **定时巡检**：基于 Cron 表达式自动执行巡检，支持邮件（HTML 完整报告）、企业微信机器人和飞书机器人通知
- **通知配置复用**：邮件收件人、企业微信 Webhook、飞书 Webhook 独立维护，多个定时任务可复用同一份通知配置
...
先访问 `/notifications` 创建通知配置（可选），集中维护邮件收件人、企业微信 Webhook 和飞书 Webhook。
...
支持选择一份通知配置，按需启用邮件通知（完整 HTML 报告）、企业微信机器人和飞书机器人。修改通知配置后，引用它的定时任务会在后续执行中自动使用最新配置。
